...
impl<F: PrimeField + Absorb> LayerProof<F> {
        ...
        E: Pairing<ScalarField = F>,
    {
        self
            .proof
            .verify::<E>(claim, num_rounds, degree_bound, transcript)
            .unwrap()
    }
}
...
impl<F: PrimeField+ Absorb> ProductCircuitEvalProof<F> {
        ...
        E: Pairing<ScalarField = F>,
    {
        ...
        for layer_id in (0..num_layers).rev() {
            ...
            let (proof_prod, rand_prod, claims_prod) = SumcheckInstanceProof::<F>::prove_cubic::<_, E>(
                &claim,
                num_rounds_prod,
        ...
        E: Pairing<ScalarField = F>,
    {
        ...
        for layer_id in (0..num_layers).rev() {
            ...
            let (proof, rand_prod, claims_prod, claims_dotp) =
                SumcheckInstanceProof::<F>::prove_cubic_batched::<_, E>(
                    &claim,
                    num_rounds_prod,
                    poly_vec_par,
                    poly_vec_seq,
