...
use ark_ec::{AffineRepr, CurveConfig, CurveGroup, short_weierstrass::{Affine, Projective, SWCurveConfig}};
...
use crate::{
    nova::commitment::CommitmentScheme,
};
...
impl<G2, C2> R1CSVar<<G2 as CurveConfig>::BaseField> for R1CSInstanceVar<G2, C2>
where
    G2: SWCurveConfig + CurveGroup,
    <G2 as CurveConfig>::BaseField: PrimeField,
    C2: CommitmentScheme<Projective<G2>>,
{
    type Value = R1CSInstance<G2, C2>;

    fn cs(&self) -> ConstraintSystemRef<<G2 as CurveConfig>::BaseField> {
        self.X
            .iter()
